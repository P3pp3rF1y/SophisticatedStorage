...
public class ModDataComponents {
	...
	public static final Supplier<DataComponentType<Map<BarrelMaterial, Identifier>>> BARREL_MATERIALS = DATA_COMPONENT_TYPES.register("barrel_materials",
			() -> new DataComponentType.Builder<Map<BarrelMaterial, Identifier>>().persistent(BarrelBlockItem.MATERIALS_CODEC)
	...
	public static final Supplier<DataComponentType<Map<Identifier, Integer>>> REMAINING_PARTS = DATA_COMPONENT_TYPES.register("remaining_parts",
			() -> new DataComponentType.Builder<Map<Identifier, Integer>>().persistent(PaintbrushItem.REMAINING_PARTS_CODEC)
	...
	private static final StreamCodec<FriendlyByteBuf, Set<Direction>> DIRECTION_SET_STREAM_CODEC = ByteBufCodecs
			.collection(size -> new HashSet<Direction>(size), Direction.STREAM_CODEC, Direction.values().length);

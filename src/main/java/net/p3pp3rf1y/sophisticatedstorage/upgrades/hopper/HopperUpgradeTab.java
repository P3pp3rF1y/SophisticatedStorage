	...
	public static class Basic extends HopperUpgradeTab {
		public Basic(HopperUpgradeContainer container, Position pos, StorageScreenBase<?> screen) {
			...
			inputFilterLogicControl = addHideableChild(
					new ContentsFilterControl.Basic(screen, new Position(x + 3, y + 24), getContainer().getInputFilterLogicContainer(),
							Config.SERVER.hopperUpgrade.inputFilterSlotsInRow.get(), StorageButtonDefinitions.STORAGE_CONTENTS_FILTER_TYPE));
			outputFilterLogicControl = addHideableChild(
					new ContentsFilterControl.Basic(screen, new Position(x + 3, inputFilterLogicControl.getY() + inputFilterLogicControl.getHeight() + 4),
							getContainer().getOutputFilterLogicContainer(), Config.SERVER.hopperUpgrade.outputFilterSlotsInRow.get(),
							StorageButtonDefinitions.STORAGE_CONTENTS_FILTER_TYPE));
		}
	}

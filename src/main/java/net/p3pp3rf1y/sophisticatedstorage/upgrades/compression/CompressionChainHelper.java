...
final class CompressionChainHelper {
	...
	static Set<Integer> getCompressionChainErrorSlots(SlotRange slotRange, Map<Integer, ItemStack> stacks) {
		...
		for (int slot = firstFilledSlot.get(); slot < slotRange.firstSlot() + slotRange.size(); slot++) {
			ItemStack stack = stacks.get(slot);
			if (stack != null && (!hasExpectedStack || !ItemStack.isSameItemSameComponents(stack, expectedStack))) {
				errorSlots.add(slot);
			}

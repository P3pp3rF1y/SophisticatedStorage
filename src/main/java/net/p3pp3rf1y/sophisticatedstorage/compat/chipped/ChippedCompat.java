	...
	public void init() {
		...
		if (ModList.get().isLoaded(CompatModIds.EMI)) {
			((Supplier<Runnable>) () -> () -> StorageEmiPlugin.addAdditionalWorkstations(registration -> {
				registration.addWorkstation(SophisticatedStorage.getRL("botanist_workbench"), ModBlocks.BOTANIST_WORKBENCH.get(),
	...
	private void registerUpgradeContainer(RegistryObject<BlockTransformationUpgradeItem> item) {
		...
		DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> UpgradeGuiManager.registerTab(containerType,
				(BlockTransformationUpgradeContainer upgradeContainer, Position position, StorageScreenBase<?> screen) -> {
					...
					return new BlockTransformationUpgradeTab(upgradeContainer, position, screen, StorageButtonDefinitions.SHIFT_CLICK_TARGET,
							StorageButtonDefinitions.REFILL_INPUT, itemName.replace('/', '_').substring(0, itemName.length() - "_upgrade".length()));
				}));
	}

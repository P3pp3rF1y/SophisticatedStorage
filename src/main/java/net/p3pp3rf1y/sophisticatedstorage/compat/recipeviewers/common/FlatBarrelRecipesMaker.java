...
public class FlatBarrelRecipesMaker {
	...
	public static List<RecipeHolder<CraftingRecipe>> getShapelessRecipes() {
		...
		recipes.add(new RecipeHolder<>(ClientRecipeHelper.recipeKey(Identifier.fromNamespaceAndPath(SophisticatedStorage.MOD_ID, "flatten_barrel")), new ShapelessRecipe("", CraftingBookCategory.MISC, flatBarrel,
				NonNullList.of(ClientRecipeHelper.emptyDisplayIngredient(), DataComponentIngredient.of(false, barrel)))));
		recipes.add(new RecipeHolder<>(ClientRecipeHelper.recipeKey(Identifier.fromNamespaceAndPath(SophisticatedStorage.MOD_ID, "unflatten_barrel")), new ShapelessRecipe("", CraftingBookCategory.MISC, barrel,

...
public class ControllerTargetHighlighter {
	...
	public static void highlightTargets() {
		...
		if (player == null || (--highlightCooldown > 0 && ItemStack.isSameItemSameComponents(lastHighlightedStack, player.getMainHandItem()))) {
			return;
		}
		...
		if (player.getOffhandItem().getItem() == ModItems.STORAGE_TOOL.get() && !player.getMainHandItem().isEmpty()) {
			...
			PacketDistributor.sendToServer(new RequestControllerTargetHighlightsPayload(player.getMainHandItem(), controllerPositions));

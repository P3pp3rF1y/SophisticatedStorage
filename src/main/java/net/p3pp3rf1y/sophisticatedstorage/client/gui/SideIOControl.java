	...
	private static class SideIOToggleButton extends ToggleButton<IOMode> {
		...
		protected List<Component> getTooltip(StateData data) {
			if (tooltip == null) {
				...
				tooltip.add(new TranslatableComponent(StorageTranslationHelper.INSTANCE.translUpgradeButton("io_mode_side_info"),
						new TranslatableComponent(StorageTranslationHelper.INSTANCE.translGui("block_side." + side.getSerializedName())),
						new TranslatableComponent(StorageTranslationHelper.INSTANCE.translGui("direction." + getDirection.apply(side).getSerializedName())))

...
public class DecorationTableScreen extends AbstractContainerScreen<DecorationTableMenu> {
	...
	private void addInheritanceArrow(int slotIndex, int xOffset, int yOffset, ButtonDefinition.Toggle<Boolean> arrowDefinition) {
		...
		addRenderableWidget(new ToggleButton<>(new Position(leftPos + slot.x + xOffset, topPos + slot.y + yOffset), arrowDefinition, button -> {
			...
			@Override
			public void renderTooltip(Screen screen, GuiGraphics guiGraphics, int mouseX, int mouseY) {
				if (getMenu().isInheritanceSlotActive(slotIndex)) {
					super.renderTooltip(screen, guiGraphics, mouseX, mouseY);
				}
			}
			...
			@Override
			public boolean mouseClicked(double mouseX, double mouseY, int button) {
				return getMenu().isInheritanceSlotActive(slotIndex) && super.mouseClicked(mouseX, mouseY, button);
			}
		});
	}
	...
	private void renderSlotOverlay(GuiGraphics guiGraphics, Slot slot, int slotColor) {
		...
		RenderSystem.setShaderColor(1, 1, 1, 1);
		guiGraphics.fillGradient(RenderType.guiOverlay(), slot.x, slot.y, slot.x + 16, slot.y + 16, 0, slotColor, slotColor);
	...
	protected void renderSlotHighlight(GuiGraphics guiGraphics, Slot slot, int mouseX, int mouseY, float partialTick) {
		if (isSlotHighlightDisabled(slot)) {
			return;
		}
	...
	private static void addPartCountInfo(Map<ResourceLocation, Integer> partCounts, List<Component> tooltip,
		...
		partCounts.forEach((part, count) -> {
			if (BuiltInRegistries.ITEM.containsKey(part)) {
				...
				BuiltInRegistries.ITEM.getTag(TagKey.create(Registries.ITEM, part)).flatMap(set -> set.stream().findFirst())
	...
	private static class PartIcon extends WidgetBase {
		...
		public void renderTooltip(Screen screen, GuiGraphics guiGraphics, int mouseX, int mouseY) {
			if (isActive.getAsBoolean() && isMouseOver(mouseX, mouseY)) {
				guiGraphics.renderTooltip(screen.getMinecraft().font, tooltip.get(), mouseX, mouseY);
			}
		}
	}
	...
	private static class BlockPreview extends CompositeWidgetBase<WidgetBase> {
		...
		protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
			...
			if (bakedModel.isCustomRenderer()) {
				IClientItemExtensions.of(previewStack).getCustomRenderer().renderByItem(previewStack, ItemDisplayContext.GUI, pose, guiGraphics.bufferSource(),

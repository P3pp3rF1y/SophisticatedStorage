...
public class DecorationTableScreen extends AbstractContainerScreen<DecorationTableMenu> {
	...
	private static void renderDisabledControlOverlay(GuiGraphicsExtractor guiGraphics, int x, int y, int width, int height, int color) {
		guiGraphics.fill(x, y, x + width, y + height, color);
	}

	private static void renderDisabledColorButtonBackground(GuiGraphicsExtractor guiGraphics, int x, int y, int width, int height) {
		guiGraphics.fill(x, y, x + width, y + height, 0xFF_A0A0A0);
	}

	private static void renderDisabledColorSwatch(GuiGraphicsExtractor guiGraphics, int x, int y, int width, int height, int color) {
		if (color == -1) {
	...
	private static void renderDisabledCheckerboard(GuiGraphicsExtractor guiGraphics, int x, int y, int width, int height) {
	...
	private void renderInputSlotPreview(GuiGraphicsExtractor guiGraphics, Slot slot) {
		...
		guiGraphics.item(previewStack, slot.x, slot.y);
		renderStackOverlay(guiGraphics, slot.x, slot.y);
	}

	private static void renderStackOverlay(GuiGraphicsExtractor guiGraphics, int x, int y) {
		...
		partCounts.forEach((part, count) -> {
			if (BuiltInRegistries.ITEM.containsKey(part)) {
				...
				BuiltInRegistries.ITEM.get(TagKey.create(Registries.ITEM, part)).flatMap(set -> set.stream().findFirst())
						.ifPresent(dye -> itemCounts.put(new ItemStack(dye), Pair.of(part, count)));
			}
		});
		...
		for (GuiEventListener child : children()) {
			if (child.isMouseOver(event.x(), event.y()) && child.mouseDragged(event, dragX, dragY)) {
				if (child instanceof BlockPreview) {
					blockPreview.stopMaterialRotation();
	...
	private static class BlockPreview extends RotatablePreviewWidget {
		...
		public void resetToDefaultRotation() {
			if (previewStacks.isEmpty() || previewStacks.get(selectedPreview).isEmpty()) {
				defaultXAxisRotation = 0;
				defaultYAxisRotation = 0;
				useDefaultReferenceRotation = true;
			...
			resolveModel(previewStacks.get(selectedPreview), renderState, ItemDisplayContext.GUI);
			ItemTransform itemTransform = getItemTransform(renderState);
			if (itemTransform == null) {
				defaultXAxisRotation = 0;
				defaultYAxisRotation = 0;
			} else {
				defaultXAxisRotation = itemTransform.rotation().x();
				defaultYAxisRotation = itemTransform.rotation().y();
			}
		...
		public void startMaterialRotation(float xAxisRotation, float yAxisRotation, float zAxisRotation) {
			...
			materialRotationFrom.set(materialRotationActive ? getMaterialRotation() : getManualRotation(currentXAxisRotation, currentYAxisRotation));
			if (!materialRotationActive && useDefaultReferenceRotation) {
				materialRotationFrom.identity();
			}
		...
		public void stopMaterialRotation() {
			materialRotationActive = false;
			useDefaultReferenceRotation = false;
		}
		...
		private static ItemTransform getItemTransform(ItemStackRenderState renderState) {
			return renderState.layers.length == 0 ? null : renderState.layers[0].itemTransform;
		}

		private Quaternionf getGuiRotation(ItemStackRenderState renderState) {
			ItemTransform itemTransform = getItemTransform(renderState);
			return itemTransform == null
					? new Quaternionf()
					: new Quaternionf().rotationXYZ((float) Math.toRadians(itemTransform.rotation().x()), (float) Math.toRadians(itemTransform.rotation().y()),
		...
		protected void renderPreview(GuiGraphicsExtractor guiGraphics, int x, int y, int width, int height, float xAxisRotation, float yAxisRotation,
			...
			guiRotation.set(getGuiRotation(renderState));
			Quaternionf manualRotation = useDefaultReferenceRotation
					? new Quaternionf()
			...
			guiGraphics.submitPictureInPictureRenderState(new DecorationTablePreviewRenderState(renderState, new Matrix3x2f(guiGraphics.pose()),
					guiGraphics.peekScissorStack(), x, y, x + width, y + previewHeight, manualRotation));
		}
	}

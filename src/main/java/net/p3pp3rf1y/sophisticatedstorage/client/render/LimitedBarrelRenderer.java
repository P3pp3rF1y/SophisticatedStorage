...
public class LimitedBarrelRenderer extends BarrelRenderer<LimitedBarrelBlockEntity> {
	...
	public void render(LimitedBarrelBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight,
		...
		if (blockEntity.isPacked() || !(blockState.getBlock() instanceof StorageBlockBase storageBlock) || (!blockEntity.hasDynamicRenderer()
				&& !blockEntity.shouldShowCounts() && !holdsItemThatShowsUpgrades() && !blockEntity.shouldShowUpgrades())) {
			return;
		}
	...
	private void renderFrontFace(LimitedBarrelBlockEntity blockEntity, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay,
		...
		if (blockEntity.hasDynamicRenderer() || holdsItemThatShowsUpgrades() || blockEntity.shouldShowUpgrades() || blockEntity.shouldShowFillLevels()

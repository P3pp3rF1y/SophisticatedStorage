...
public class StorageBlockPreviewPipRenderer extends PictureInPictureRenderer<StorageBlockPreviewRenderState> {
	...
	protected void renderToTexture(StorageBlockPreviewRenderState renderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector) {
		...
		StorageBlockPreviewRenderer.submitStorageBlock(renderState.storageBlockEntity(), renderState.partialTicks(), poseStack, submitNodeCollector, 15728880);
		if (renderState.otherStorageBlockEntity() != null && otherStorageOffset != null) {
			...
			StorageBlockPreviewRenderer.submitStorageBlock(renderState.otherStorageBlockEntity(), renderState.partialTicks(), poseStack, submitNodeCollector,
					15728880);
			poseStack.popPose();
		}
	}
	...
	private void setupPreviewLighting() {
		if (previewLightingBuffer == null) {
			...
			try (MemoryStack memoryStack = MemoryStack.stackPush()) {
				ByteBuffer buffer = Std140Builder.onStack(memoryStack, Lighting.UBO_SIZE).putVec3(ITEM_DISPLAY_PREVIEW_LIGHT_0)
						.putVec3(ITEM_DISPLAY_PREVIEW_LIGHT_1).get();
				RenderSystem.getDevice().createCommandEncoder().writeToBuffer(previewLightingBuffer.slice(0L, Lighting.UBO_SIZE), buffer);
			}
		}

		RenderSystem.setShaderLights(previewLightingBuffer.slice(0L, Lighting.UBO_SIZE));
	}

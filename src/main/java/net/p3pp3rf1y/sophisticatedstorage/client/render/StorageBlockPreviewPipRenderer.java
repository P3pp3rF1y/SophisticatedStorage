...
public class StorageBlockPreviewPipRenderer extends PictureInPictureRenderer<StorageBlockPreviewRenderState> {
	...
	protected void renderToTexture(StorageBlockPreviewRenderState renderState, PoseStack poseStack) {
		...
		StorageBlockPreviewRenderer.submitStorageBlock(renderState.storageBlockEntity(), renderState.partialTicks(), poseStack, submitNodeStorage, 15728880);
	...
	private void setupPreviewLighting() {
		if (previewLightingBuffer == null) {
			...
			try (MemoryStack memoryStack = MemoryStack.stackPush()) {
				ByteBuffer buffer = Std140Builder.onStack(memoryStack, Lighting.UBO_SIZE).putVec3(ITEM_DISPLAY_PREVIEW_LIGHT_0).putVec3(ITEM_DISPLAY_PREVIEW_LIGHT_1)
						.get();

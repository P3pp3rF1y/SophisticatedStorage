...
public class RenderHelper {
	...
	private static TextureAtlasSprite parseSprite(ResourceLocation blockName, @Nullable Direction direction, Random rand) {
		...
		if (sprite == null) {
			sprite = Minecraft.getInstance().getModelManager().getMissingModel().getParticleIcon(EmptyModelData.INSTANCE);
		}
	...
	private static TextureAtlasSprite parseSpriteFromModel(BlockState blockState, @Nullable Direction direction, Random rand) {
		...
		try {
			List<BakedQuad> culledQuads = blockModel.getQuads(blockState, direction, rand, EmptyModelData.INSTANCE);
		...
		if (sprite == null) {
			try {
				sprite = blockModel.getParticleIcon(EmptyModelData.INSTANCE);
			}
			catch (Exception e) {
				// NO OP
			}

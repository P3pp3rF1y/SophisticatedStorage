...
class StorageRecipeViewerDisplaySpecTest {
	...
	void tierUpgradeUsagePreservesFocusedWoodBarrelComponents() {
		...
		assertTrue(tierUsages.stream().anyMatch(usage -> ItemStack.isSameItemSameComponents(woodStorageStack(ModBlocks.COPPER_BARREL_ITEM.get(), WoodType.SPRUCE), usage.firstOutput())));
		assertTrue(tierUsages.stream().anyMatch(usage -> ItemStack.isSameItemSameComponents(woodStorageStack(ModBlocks.IRON_BARREL_ITEM.get(), WoodType.SPRUCE), usage.firstOutput())));
	...
	void flatBarrelUsageShowsUnflattenRecipe() {
		...
		List<CraftingDisplayVariant> usages = getCraftingUsagesFor(catalog, flatSpruceBasicBarrel).stream()
				.filter(usage -> ItemStack.isSameItemSameComponents(spruceBasicBarrel, usage.firstOutput()))
				.toList();
	...
	void tintedFlatBarrelUsageHasSourceIngredientSlot() {
		...
		List<CraftingDisplayVariant> usages = getCraftingUsagesFor(catalog, tintedBarrel).stream()
				.filter(usage -> ItemStack.isSameItemSameComponents(flatTop(tintedBarrel), usage.firstOutput()))
				.toList();
	...
	void tierUsageChainCanReachNetheriteUpgrade() {
		...
		assertTrue(ItemStack.isSameItem(new ItemStack(ModBlocks.NETHERITE_BARREL_ITEM.get()), netheriteUsages.getFirst().firstOutput()), "Expected netherite barrel but got " + netheriteUsages.getFirst().firstOutput());
	}
	...
	private static CraftingDisplaySpec getFlatBarrelSpec(IRecipeViewerDisplayCatalog catalog) {
		return catalog.getCraftingSpecs().stream()
				.filter(spec -> spec.id().equals(SophisticatedStorage.getRL("flat_top_barrel_toggle")))
				.findFirst()
				.orElseThrow();
	}

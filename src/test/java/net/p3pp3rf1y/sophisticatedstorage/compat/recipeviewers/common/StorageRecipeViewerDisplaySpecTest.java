...
class StorageRecipeViewerDisplaySpecTest {
	...
	void tierUpgradeFocusPreservesFlatWoodBarrelComponents() {
		...
		List<CraftingDisplayVariant> usages = getCraftingUsagesFor(catalog, flatAcaciaBarrel).stream().filter(usage -> usage.inputs().size() > 1).toList();
		List<CraftingDisplayVariant> recipes = getCraftingRecipesFor(catalog, flatAcaciaIronBarrel).stream().filter(recipe -> recipe.inputs().size() > 1)
				.toList();

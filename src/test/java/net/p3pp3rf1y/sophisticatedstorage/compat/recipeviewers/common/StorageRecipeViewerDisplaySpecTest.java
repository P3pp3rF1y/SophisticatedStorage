...
class StorageRecipeViewerDisplaySpecTest {
	...
	void catalogOwnsExactTierUpgradeRecipeReplacement() {
		...
		RecipeHolder<?> replacedRecipe = new RecipeHolder<>(ClientRecipeHelper.recipeKey(Identifier.fromNamespaceAndPath(SophisticatedStorage.MOD_ID, "iron_barrel")), spec.recipeHolder(spec.getGlobalDisplays().getFirst()).value());
		RecipeHolder<?> unrelatedRecipe = new RecipeHolder<>(ClientRecipeHelper.recipeKey(Identifier.parse("test:unrelated")), spec.recipeHolder(spec.getGlobalDisplays().getFirst()).value());
	...
	private static ItemStack findUsageOutput(IRecipeViewerDisplayCatalog catalog, ItemStack stack, ItemStack expectedOutput) {
		return getCraftingUsagesFor(catalog, stack).stream()
				.map(CraftingDisplayVariant::firstOutput)
				.filter(output -> ItemStack.isSameItem(output, expectedOutput))
				.findFirst()
				.orElseThrow();
	}

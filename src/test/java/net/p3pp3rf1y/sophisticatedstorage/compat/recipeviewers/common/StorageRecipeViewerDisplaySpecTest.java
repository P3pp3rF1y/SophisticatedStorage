...
class StorageRecipeViewerDisplaySpecTest {
	...
	void genericWoodStorageRecipesPreserveLoadedNonWoodIngredients() {
		try (MockedStatic<ClientRecipeHelper> clientRecipeHelper = Mockito.mockStatic(ClientRecipeHelper.class, Mockito.CALLS_REAL_METHODS)) {
			...
			List<Optional<Ingredient>> inputs = List.copyOf(RecipeViewerRecipeHelper.getIngredients(oakRecipe.value()));
	...
	private static RecipeHolder<GenericWoodStorageRecipe> customGenericChestRecipeHolder(Item nonWoodIngredient) {
		...
		ShapedRecipe recipe = new ShapedRecipe(new Recipe.CommonInfo(true), new CraftingRecipe.CraftingBookInfo(CraftingBookCategory.MISC, ""),
				new ShapedRecipePattern(3, 3, ingredients.stream().map(Optional::of).toList(), Optional.empty()), ItemStackTemplate.fromNonEmptyStack(result));
		return new RecipeHolder<>(ClientRecipeHelper.recipeKey(Identifier.parse("test:custom_generic_chest")), new GenericWoodStorageRecipe(recipe));
	}

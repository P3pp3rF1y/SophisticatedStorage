...
class StorageRecipeViewerDisplaySpecTest {
	...
	void catalogOwnsExactTierUpgradeRecipeReplacement() {
		...
		CraftingDisplaySpec spec = catalog.getCraftingSpecs().stream()
				.filter(craftingSpec -> craftingSpec.replacesCraftingRecipe(
						new RecipeHolder<>(ClientRecipeHelper.recipeKey(ResourceLocation.fromNamespaceAndPath(SophisticatedStorage.MOD_ID, "iron_barrel")),
								craftingSpec.recipeHolder(craftingSpec.getGlobalDisplays().getFirst()).value())))
	...
	void genericWoodStorageRecipesPreserveLoadedNonWoodIngredients() {
		try (MockedStatic<ClientRecipeHelper> clientRecipeHelper = Mockito.mockStatic(ClientRecipeHelper.class, Mockito.CALLS_REAL_METHODS)) {
			...
			List<Optional<Ingredient>> inputs = List.copyOf(RecipeHelper.getIngredients(oakRecipe.value()));
			assertTrue(inputs.get(0).orElseThrow().test(new ItemStack(Items.OAK_PLANKS)));
			assertFalse(inputs.get(0).orElseThrow().test(new ItemStack(Items.SPRUCE_PLANKS)));
			assertTrue(inputs.get(4).orElseThrow().test(new ItemStack(Items.STONE_BUTTON)));
			assertFalse(inputs.get(4).orElseThrow().test(new ItemStack(Items.LEVER)));
		}
	}
	...
	private static RecipeHolder<GenericWoodStorageRecipe> customGenericChestRecipeHolder(Item nonWoodIngredient) {
		...
		ShapedRecipe recipe = new ShapedRecipe("", CraftingBookCategory.MISC,
				new ShapedRecipePattern(3, 3, ingredients.stream().map(Optional::of).toList(), Optional.empty()), result);
		return new RecipeHolder<>(ClientRecipeHelper.recipeKey(ResourceLocation.parse("test:custom_generic_chest")), new GenericWoodStorageRecipe(recipe));
	}
				...
				.thenAnswer(invocation -> {
					...
					return ClientRecipeHelper.transformAllRecipeHoldersOfType(recipeManager, recipeType, recipeClass,
							(id, recipeHolder) -> invocation.<java.util.function.BiFunction<ResourceLocation, RecipeHolder, ?>>getArgument(2).apply(id,
									normalizeRecipeHolder(recipeHolder, resources)));
				});
	...
	private static RecipeHolder<?> normalizeRecipeHolder(RecipeHolder<?> recipeHolder, TestRecipeResources.LoadedResources resources) {
		if (recipeHolder.value() instanceof CraftingRecipe recipe && RecipeHelper.getIngredients(recipe).stream().flatMap(Optional::stream)
				.anyMatch(ingredient -> ingredient.test(new ItemStack(ModBlocks.DIAMOND_BARREL_ITEM.get())))) {
